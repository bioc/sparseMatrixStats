...
setMethod("rowSums2", signature(x = "xgCMatrix"),
  ...
  if(is.logical(cols) && length(cols) == ncol(x)){
    set_result_names_t(dgCMatrix_rowSums2_col_select(x, na_rm = na.rm, col_selector = cols), useNames)
  }else{
    if(! is.null(cols)){
      x <- x[, cols, drop = FALSE]
    }
...
setMethod("rowVars", signature(x = "xgCMatrix"),
  ...
  if(! is.null(center) && length(center) != nrow(x)){
    stop("Argument 'center' must be the same length as number of rows of 'x': ", length(center), "!=", nrow(x))
  }
  if(! is.null(rows)){
    x <- x[rows, , drop = FALSE]
  }
